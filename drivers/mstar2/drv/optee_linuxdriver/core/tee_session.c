...
static char *_uuid_to_str(const TEEC_UUID *uuid)
{
	...
	if (uuid) {
		snprintf(uuid_str, _UUID_STR_SIZE,
		...
		snprintf(uuid_str, _UUID_STR_SIZE, "NULL");
...
int tee_session_invoke_be(struct tee_session *sess, struct tee_cmd_io *cmd_io)
{
	...
	TIME_PRINTK_START("TEE_INVOKE COPY FROM USER ENTER", start, 0);
	ret = _init_tee_cmd(sess, cmd_io, &cmd);
	if (ret)
		goto out;
	TIME_PRINTK_END("TEE_INVOKE COPY FROM USER EXIT", start, end, 0);
	ret = tee->ops->invoke(sess, &cmd);
	TIME_PRINTK_START("TEE_INVOKE COPY TO USER ENTER", start, 0);
	if (!ret)
		_update_client_tee_cmd(sess, cmd_io, &cmd);
	else {
		/* propagate the reason of the error */
	...
	TIME_PRINTK_END("TEE_INVOKE COPY TO USER EXIT", start, end, 0);
out:
	_release_tee_cmd(sess, &cmd);
			      ...
			      unsigned long arg)
{
	...
	case TEE_INVOKE_COMMAND_IOC:
		TIME_PRINTK_START("TEE_INVOKE_COMMAND_IOC ENTER", start, 0);
		ret =
		    tee_do_invoke_command(sess,
					  (struct tee_cmd_io __user *)arg);
		TIME_PRINTK_END("TEE_INVOKE_COMMAND_IOC EXIT", start, end, 0);
		break;
	case TEE_REQUEST_CANCELLATION_IOC:
		TIME_PRINTK_START("TEE_REQUEST_CANCELLATION_IOC ENTER", start, 0);
		ret = tee_do_cancel_cmd(sess, (struct tee_cmd_io __user *)arg);
		TIME_PRINTK_END("TEE_REQUEST_CANCELLATION_IOC EXIT", start, end, 0);
		break;
	default:
		ret = -ENOSYS;
		break;
...
int tee_session_close_and_destroy(struct tee_session *sess)
{
	...
	TIME_PRINTK_END("TEE_SESSIOB_CLOSE_AND_DESTRORY EXIT", start, end, 0);
	dev_dbg(_DEV(tee), "%s: <\n", __func__);
	return ret;
}
				   ...
				   struct tee_cmd *cmd)
{
	...
	if (tee_context_copy_from_client(ctx, &_op, cmd_io->op, sizeof(_op))) {
		dev_err(_DEV_TEE,
			"%s:%d: can't get client op\n",
			__func__, __LINE__);
		return;
	}
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		case TEEC_MEMREF_TEMP_INOUT:
			...
			if (cmd_io->err == TEEC_ERROR_SHORT_BUFFER || size_new > _op.params[idx].tmpref.size)
#else
			if (size_new > _op.params[idx].tmpref.size)
#endif
				dev_err(_DEV_TEE,
		...
		case TEEC_MEMREF_WHOLE:
			...
			if (type == TEEC_MEMREF_WHOLE) {
				...
				offset = _op.params[idx].memref.offset;
				size = _op.params[idx].memref.size;
			}
