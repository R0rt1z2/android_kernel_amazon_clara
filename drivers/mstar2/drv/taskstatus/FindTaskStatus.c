...
{
	iTASK_PID = pid;
	printk("Set_Target_pid :(tgid,iTASK_PID_OOM)=(%d, %d)\n", iTASK_PID, iTASK_PID_OOM);
	return 0;
}
...
int Delete_Target_pid(void)
{
	...
	printk("Delete_Target_pid :(tgid,iTASK_PID_OOM)=(%d,%d)\n", iTASK_PID, iTASK_PID_OOM);
	return 0;
}
...
int Set_Target_pid_OOM_adj(int oom_adj)
{
	...
	printk("Set_Target_pid_OOM_adj :(tgid,iTASK_PID_OOM)=(%d,%d) \n", iTASK_PID, iTASK_PID_OOM);
	return 0;
}
EXPORT_SYMBOL(Set_Target_pid_OOM_adj);
int Get_Target_pid_OOM_adj(void)
{
	printk("Get_Target_pid_OOM_adj :(tgid,iTASK_PID_OOM)=(%d,%d) \n", iTASK_PID, iTASK_PID_OOM);
...
static ssize_t FindTask_list_write(struct file *file, const char __user *buffer,
	...
	return -EFAULT;
	if (strlen((TaskStatus_name_list) + strlen(local_buf)) <= 64) {
		strcat(TaskStatus_name_list, local_buf);
		return count;
	} else {
		return -EFAULT;
	}
...
static ssize_t FindTask_list_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	printk("TaskStatus_list : (tgid,stauts)=(%d,%d) \n", pid, oom);
	return 0;
}
...
#define SEPARATE_SYMBOL ":"
	char *base = kzalloc(LIST_MAX_LENGTH, GFP_ATOMIC);
	char *base_ptr = NULL;
	...
	if (len > LIST_MAX_LENGTH)
		len = LIST_MAX_LENGTH;
	strncpy(base_ptr, searchSource, len);
	while ((pch = strsep(&base_ptr, SEPARATE_SYMBOL)) != NULL) {
