...
void MIRC_support_ir_max_timeout(void)
{
    ...
    {
        u32 temp = dev->support_ir[i].u32IRSpeed *(ir_map_period(dev->support_ir[i].eIRType)) + 20;
        if(temp > eventtimeoutMax){
            printk("eventtimeoutMax is %d\n",eventtimeoutMax);
            eventtimeoutMax=temp;
        }    
    }
    //return eventtimeoutMax;
}
...
int MIRC_Data_Ctrl_Init(struct mstar_ir_dev *dev)
{
	...
	dev->ws = wakeup_source_register("PowerKeyPress");
	if (!dev->ws) {
		ret = -ENOMEM;
		goto out;
	}
...
static u32 MIRC_Keycode_From_Map(u32 keymapnum,u32 scancode)
{
    ...
    {
	/*Need to consided the case when header is 0, such as Greta's */
...
static void MIRC_Do_Keydown(struct mstar_ir_dev *dev, int scancode,u32 keycode)
{
	...
	/* Cancel PM suspend on power key press */
	if (new_event && keycode == KEY_POWER) {
		pr_info("ir_core: report wakeup event on power key press.\n");
		__pm_wakeup_event(dev->ws, 5 * MSEC_PER_SEC);
	}
