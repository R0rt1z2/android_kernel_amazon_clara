...
extern void usb_dump_ehci_debug_register(struct usb_hcd *hcd)
{
	...
	for (i = 0; i < 2; i++) {
		reg_84 = hcd_readb(hcd, 0x84);
		reg_85 = hcd_readb(hcd, 0x85);
		reg_86 = hcd_readb(hcd, 0x86);
		reg_87 = hcd_readb(hcd, 0x87);
		reg_88 = hcd_readb(hcd, 0x88);
		reg_89 = hcd_readb(hcd, 0x89);
		printk(KERN_INFO "[%d]HC_DBG_REG:%02X,%02X,%02X,%02X,%02X,%02X\n", i,
			reg_84, reg_85, reg_86, reg_87, reg_88, reg_89);
	}
}
#endif
