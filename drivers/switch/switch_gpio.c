...
static unsigned int hp_en_used;
static unsigned int hp_en_inverse;
static unsigned int hp_en_gpio;
static unsigned int hp_en_value;
...
extern int get_headphone_state(void);
...
static void gpio_poll(void* arg)
{
	...
	while(1)
	{
		...
		if (hp_en_used) {
			hp_en_value = gpio_get_value(hp_en_gpio);
			if (switch_data->inverse != 0) {
				if (hp_en_value == state) {
					gpio_set_value(hp_en_gpio, !state);
					pr_info("[%s][%d] gpio_set_value HP_EN_IC = %d\n", __FUNCTION__, __LINE__, !state);
				}
			} else {
				if (hp_en_value != state) {
					gpio_set_value(hp_en_gpio, state);
...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	if (dn)
	{
		...
		of_property_read_u32(dn, "switch-shared", &dn_switch_shared);
	}
	else
	{
...
#if defined (CONFIG_OF)
	switch_data->shared = dn_switch_shared;
	pr_info("Headphone %s shared with other device\n", dn_switch_shared ? "is":"is not");
...
static int __init hp_en_init(void)
{
	int ret = 0;
	struct device_node *np;
	u32 prop;
	hp_en_used = 0;
	np = of_find_node_by_name(NULL, "hp_en_gpio");
	if (np == NULL) {
		pr_err("hp_en_gpio is not defined in dts \n");
		goto err;
	}

	if (!of_property_read_u32(np, "hp_en-gpio", &prop)) {
		hp_en_gpio = prop;
		hp_en_used = 1;
		pr_info("hp_en-gpio is %d \n", hp_en_gpio);
	} else {
		pr_err("hp_en-gpio is not defined \n");
		goto err;
	}

	if (!of_property_read_u32(np, "hp_en-inverse", &prop)) {
		hp_en_inverse = prop;
		pr_info("hp_en-inverse is %d \n", hp_en_inverse);
	} else {
		pr_err("hp_en-inverse is not defined \n");
		goto err;
	}
err:
	return ret;
}
early_initcall(hp_en_init);
